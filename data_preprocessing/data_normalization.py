...
def normalize_data(training_features, test_features):
    '''
	Normalize corrosion depth and concrete property features on training set. Apply the same normalization to the test set. This is stable so for inference, this function should be called with the same training data, and the inference dataset.

    Args:
		training_features: Numpy array of shape (num_train_samples, 343). Each row contains: simulation_idx, timestep, 4 concrete property featurs, and 337 corrosion depth features.
		test_features: Numpy array of shape (num_test_samples, 343). Same as training_features.
